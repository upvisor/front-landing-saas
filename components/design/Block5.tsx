        ...
        <div className="w-full text-center max-w-[1280px] m-auto flex flex-col gap-6">
          {
            ...
            </div>
            {
              content.info?.image && content.info.image !== ''
                ? <Image className={`h-fit mx-auto mt-4`} style={{ boxShadow: style.design === 'Sombreado' ? `0px 3px 20px 3px ${style.borderColor}10` : '', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '', border: style.design === 'Borde' ? `1px solid ${style.borderColor}` : '' }} width={480} height={300} alt='Imagen slider prueba' src={content.info.image} />
                : ''
            }
        </div>
      </div>
