...
export const Block4 = ({ content, index, calls, forms, design, payment, style }: { content: IDesign, index: any, forms: IForm[], calls: ICall[], design: Design, payment: IPayment, style?: any }) => {
        ...
        <div className="w-full text-center max-w-[1280px] m-auto flex flex-col gap-6">
          ...
          <div className="flex gap-4 flex-col md:flex-row">
            ...
            <div className="w-full flex flex-col gap-3 md:w-1/3">
              ...
              {
              content.info.buttonLink3 === 'Abrir popup' || calls.find(call => call._id === content.info.buttonLink3) || forms.find(form => form._id === content.info.buttonLink3)
                ? <Button action={(e: any) => {
                  e.preventDefault()
          ...
          </div>
          {
            content.info?.image && content.info.image !== ''
              ? <Image className='h-fit m-auto' style={{ boxShadow: style.design === 'Sombreado' ? '0px 3px 20px 3px #11111110' : '', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '' }} width={480} height={300} alt='Imagen slider prueba' src={content.info.image} />
              : ''
          }
        </div>
      </div>
